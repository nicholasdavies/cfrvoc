...
do_table = function(dataS, what)
{
    ...
    names(result) = c("Characteristic", 
        paste0("Miss (N = ",   prettyNum(sum(tbl_miss$N), big.mark = ","), ")"), 
...
dataS[, table(sgtf, useNA = "ifany")]
tbl_miss = dataS[is.na(sgtf), .N, keyby = .(characteristic = Sex)][, .(characteristic, N, pct = 100 * N / sum(N))]
...
fwrite(final, "./output/table1_miss.csv")
...
fwrite(final_deaths, "./output/table2_miss.csv")
